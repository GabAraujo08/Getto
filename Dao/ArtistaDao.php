...
class ArtistaDao
{
    ...
    public static function buscaArtistasAtivos()
    {
        $conexao = Conexao::conectar();
        $consulta = $conexao->prepare('
        select u.*, a.bioArtista, a.idArtista, count(p.idPublicacao) as qtdPosts from tbpublicacao p
        join tbartista a ON
        p.idArtista = a.idArtista
        join tbusuario u ON
        a.idUsuario = u.idUsuario
        group by p.idArtista
        order by count(p.idPublicacao) desc
                ');
